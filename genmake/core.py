...
class GenMakeConfig:
    ...
    def __init__(self, **kwargs):
        """
        """
        _ARGUMENTS = {"author": self.author_set,
        ...
        for key in _ARGUMENTS:
            ...
            if key in kwargs:
                _ARGUMENTS[key](kwargs[key])
            # Otherwise let the mutator function use default value
            # NOTE: 'directories_set' would raise exception with default 'None'
            else:
                _ARGUMENTS[key]()

    def author_set(self, author=None):
        """
        """
        if None == author:
            self.config["author"] = _author_get()
            return

        if isinstance(author, str) and author and author.isidentifier():
            self.config["author"] = author
        else:
            raise ValueError("'author' argument must be non-empty 'str' type")
